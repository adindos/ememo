...
class BudgetController extends AppController{
	...
	public function dashboard($budgetid){
		...
		if($isConfirm['Budget']['submission_no'] == 0){
			$this->Session->setFlash('This budget is not yet confirmed <br><small>Please click the <strong><em>Next</em></strong> button to confirm the budget </small>','flash.error');
	...
	public function allrequest(){
		...
		if ($user['role_id']==17 || $user['role_id']==18){
			...
			$budget = $this->Budget->find('all',array(
								'conditions'=>array('Budget.department_id'=>$user['department_id'],'Budget.submission_no NOT'=>0),
								'order'=>array('Budget.created DESC')));
		}
		$this->set('budget',$budget);
	}
	...
	public function request($budgetid){
		...
		if(!$this->isRequestor($budgetid) || !$user['role_id']==18){
	...
	*/
	...
	public function confirmBudget(){
		...
		if($this->request->is(array('post','put'))){
			...
			$budget['Budget']['Department'] = $budget['Department'];
			// unset($budget['User']);
	...
	*/
	...
	public function sendReviewEmail($budgetid,$approvedBy=null){
		...
		if(empty($status)){ // send to requestor
			...
			$toRequestor = $budget['Budget']['user_id'];
			$subject = $budget['Budget']['title']." (Budget request approved)";
			...
			if($status['BReviewer']['sequence'] != 1){
				...
				$subject = $status['Budget']['title']." (Budget reviewed)";
			...
			$subject = $budget['Budget']['title']." (Please review the budget request)";
	...
	public function sendRejectedEmail($budgetid){
		...
		$toRequestor = $budget['Budget']['user_id'];
		$subject = $budget['Budget']['title']." (Budget request rejected)";
	...
	private function budgetTable($budgetid,$budgetData = array()){
		...
		$htmlTable = "<table style='width:90%;border-collapse:collapse;padding: 10px 15px;margin:20px;background:#fff;border:1px solid #d9d9d9' border='1' cellpadding='6'>
						...
						<tr>
							<td style='background:#777;color:#FFF;font-weight:bold;padding: 10px;'> Year </td>
	...
	public function budgetExcel($budgetid){
		...
		$this->layout = 'mems-excel';
        $this->excelConfig =  array(
            'filename' => $budget['Budget']['title'].'_'.$budget['Budget']['year'].'.xlsx'
        );
	}
