					...
					<tbody>
					   ...
					   if (!empty($memo)):
					   	foreach ($memo as $key=>$m){
							...
							foreach($m['NfMemo']['NfStatus'] as $mstatus):
						...
						<tr>
						   ...
						   ?>
							...
							<td class="text-center"><?php echo $m['NfMemo']['Department']['department_name'] ?></td>

							<td class="text-center"><?php echo $m['NfMemo']['Department']['Group']['group_name'] ?></td>
							...
							<td class="text-center">
								<?php
								$commentFlag=false;
								if (!empty($m['NfMemo']['NfComment'])){
									$commentFlag=true;
									
								}
									if($commentFlag)
										echo $this->Html->link('<em> Comment </em>',array('controller'=>'Comment','action'=>'index',$m['NfMemo']['memo_id'],'nonfinancial'),array('escape'=>false,'class'=>'btn btn-info btn-xs tooltips data-toggle="tooltip" data-placement="top" data-original-title="Comment" '));
									else
										echo "<em> None </em>";
								?>
							</td>
							...
							<td class="text-center">
								<!-- <div class="btn-group btn-group-xs"> -->
									<?php
										...
										echo $this->Html->link('<i class="fa fa-dashboard"></i> Dashboard',array('controller'=>'NfMemo2','action'=>'dashboard',$m['NfMemo']['memo_id']),array('escape'=>false,'class'=>'btn bg-primary btn-xs tooltips data-toggle="tooltip" data-placement="top" data-original-title="Dashboard" '));
