<?php
	$this->Html->addCrumb('Reports', array('controller' => 'report', 'action' => 'financialMemoReport'));
	$this->Html->addCrumb('Financial Memo Report', $this->here,array('class'=>'active'));
?>
				...
				<div class="panel-body">
					<?php
	                    ...
	                    echo $this->Form->create('Filter', array(
	                        'url' => array('controller' => 'report' , 'action' => 'financialMemoReport'),'id'=>'reportForm'));
	                 ?>
					<div class="col-lg-1">
					</div>
					<div class="col-lg-5">
					   ...
					   if (!empty($memo)):
						...
						<tr>
						   ...
						   <?php 
						   	...
						   	echo '<b>'.$this->Html->link($m['FMemo']['subject'],array('controller'=>'FMemo','action'=>'dashboard',$m['FMemo']['memo_id']),array('escape'=>false)).'</b>';
...
<script type="text/javascript" charset="utf-8">
        $('#pdf').on('click',function(){
            $('#reportForm').attr('action','<?php echo ACCESS_URL ?>report/exportFMemo/pdf.pdf');
        });
        $('#excel').on('click',function(){
            $('#reportForm').attr('action','<?php echo ACCESS_URL ?>report/exportFMemo/excel.xlsx');
        });
        $('#filter').on('click',function(){
            $('#reportForm').attr('action','<?php echo ACCESS_URL ?>report/financialMemoReport');
        });
